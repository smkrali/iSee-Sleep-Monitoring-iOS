...
//    var work:[CDouble] = [CDouble](count:lwork, repeatedValue:0)
    ...
    var nc1 = __CLPK_integer(N)
    var nc2 = nc1
    var nc3 = nc1
    dgetrf_(&nc1, &nc2, !y, &nc3, &ipiv, &info)
    dgetri_(&nc3, !y, &nc1, &ipiv, &work, &lwork, &info)
    return y
}
...
func eig(_ x: matrix)->vector{
    ...
    var nc2 = nc1
    var nc3 = nc1
    dgeev_(&jobvl, &jobvr, &nc, !x, &nc1,
        !value_real, !value_imag, !vector, &nc2, !vector, &nc3,
        &work, &lwork, &info)
